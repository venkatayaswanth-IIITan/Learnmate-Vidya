...
import { getInitialModelMetrics } from "../utils/metricsService";
...
export default function StudentProfile() {
    ...
    // Sync profile stats whenever roadmap changes
    const syncProfileStats = (data) => {
        if (!data || !data.roadmap) return;
        const completed = data.roadmap.filter(r => r.status === 'completed').length;
        setProfile(prev => ({
            ...prev,
            roadmap: {
                ...prev.roadmap,
                completedTasks: completed,
                totalTasks: data.roadmap.length
            }
        }));
    };
        ...
        <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Card title="Activity Status">
                    <div className="flex items-center justify-between h-full pb-4 gap-2">
                        <div>
                        ...
                        <div className="text-right">
                            <p className="text-xs text-slate-400 font-medium italic">Data synced from recent</p>
                            <p className="text-xs text-indigo-500 font-bold uppercase tracking-tighter">Learning Logs</p>
                        </div>
                    </div>
                </Card>
                ...
                <Card title="Recent Activity" className="md:col-span-1">
                    ...
                    <p className="text-xs text-slate-400 mt-3 text-right">Last 14 Days Heatmap</p>
                </Card>
            </div>
        </div>
    );
